...
*Interpretation:* Out-of-range values are set to NA before summing;
scores with NA items return NA.
...
- Non-numeric inputs: coerced with warnings; resulting NAs propagate
  unless `omit` is used.
- Out-of-range values: a warning is emitted; recode out-of-range items
  to NA before calling.
