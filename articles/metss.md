...
- `check_extreme`: set TRUE to scan inputs; `extreme_action`
  (`warn`/`cap`/`error`/`ignore`/`NA`) controls handling. Defaults (cm,
  mmHg, mmol/L): waist 40–200; bp_sys 60–300; bp_dia 30–200; TG 0–20;
  HDL_c 0–5; glucose 0–40. Override with `extreme_rules`.
...
- Extreme handling can cap/NA/warn/error; diagnostics run independently
  and can still warn after capping.
