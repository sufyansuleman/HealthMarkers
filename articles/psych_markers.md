...
Compute common self-report mental health scales (PHQ-9/8, GAD-7, K6/K10,
GHQ-12, WHO-5, ISI, MDQ, ASRS), key-driven scales (BIS, SPQ), cognitive
composites, and diagnosis/medication flags in one call via
[`psych_markers()`](https://sufyansuleman.github.io/HealthMarkers/reference/psych_markers.md).
NA handling (`keep`/`omit`/`error`) and rowwise mean-imputation are
built in, with per-scale missingness thresholds.

## Minimal example data

Here we build a tiny synthetic row with required item columns. Replace
...
Map canonical item IDs to your dataset columns. Keys for BIS/SPQ stay
...
Select the scales you want via `which`. Rowwise mean imputation is
optional; here we keep defaults and missingness tolerance of 20% per
scale.
...
#> #   cog_z_mean <dbl>, dx_any_psych <lgl>, dx_internalizing <lgl>, …
```
...
- `na_action`: `keep` (default), `omit` (drops rows with NA in new
  outputs), `error`.
- `missing_prop_max`: rows with higher missing proportion per scale are
  set to `NA` for that scale.
- `impute = "mean"` applies rowwise mean-imputation before scoring; use
  `"none"` to skip.
- BIS/SPQ require user-supplied keys; no item text ships in the package.
- Cognitive composite supports `method = "z_mean"` or `"pca1"` (complete
  cases only for PCA loadings).
