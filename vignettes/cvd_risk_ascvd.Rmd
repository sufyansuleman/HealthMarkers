...
## Scope
Run the ASCVD Pooled Cohort Equations (10- or 30-year) with guarded optional dependency loading and clear input expectations.

## When to use
- You need ACC/AHA Pooled Cohort ASCVD risk on U.S. adults with standard clinical inputs.
...
Use a small subset (e.g., first 30 rows) if sampling from the simulated data: select `age`, `sex`, `race`, `smoker`, `total_chol`, `HDL_c`, `sbp`, `bp_treated`, `diabetes`, `bmi`.
...
ascvd_10 <- cvd_risk_ascvd(data = ascvd_data, year = 10, verbose = FALSE)

head(ascvd_10)
```

## 30-year ASCVD risk
Switch the horizon to 30 years. The same inputs are reused.

```{r eval = requireNamespace("PooledCohort", quietly = TRUE)}
ascvd_30 <- cvd_risk_ascvd(data = ascvd_data, year = 30)
...
- Use `year = 10` or `30`; other values are rejected.
