...
```{r}
df_na <- tibble(NfL = c(8.5, NA, 22.1))
...
```{r, warning=TRUE}
df_ext <- tibble(NfL = c(8.5, -5, 22.1))  # negative value is out of range
...
```{r}
nfl_marker(
  data = tibble(NfL = c(8.5, 5000, 22.1)),
  col_map = cm,
  check_extreme = TRUE,
  extreme_action = "cap",
...
nfl_marker(tibble(NfL = c(8.5, 14.2, 22.1)), cm, verbose = TRUE)
options(old_opt)
```
