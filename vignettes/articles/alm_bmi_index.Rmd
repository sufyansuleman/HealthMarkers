...
- Policies: `na_action` (`keep`/`warn`/`ignore` behave like keep; `omit` drops; `error` aborts).
...
2. **Column map** — confirms which data column each required key (`alm`, `bmi`, `sex`) resolved to.
   Example: `alm_bmi_index(): column map: alm -> 'ALM_kg', bmi -> 'BMI', sex -> 'Sex'`
3. **Results summary** — shows how many rows computed successfully (non-NA) per output column.
   Example: `alm_bmi_index(): results: alm_bmi_index 30/30, low_alm_bmi_flag 30/30, ...`
...
Run `hm_col_report(your_data)` to check which body composition columns are
