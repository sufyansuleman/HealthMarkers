...
2. **Column map** — lists each variable passed via `vars`.
   Example: `calc_sds(): column map: BMI -> 'BMI', sbp -> 'sbp'`
3. **Results summary** — shows how many rows computed successfully (non-NA) per `_sds` output column.
   Example: `calc_sds(): results: BMI_sds 28/30, sbp_sds 30/30, ...`
