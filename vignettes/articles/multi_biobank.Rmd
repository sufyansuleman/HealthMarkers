...
The Danish laboratory information system uses **NPU codes** (Nordic-Baltic/IFCC
nomenclature). All primary NPU codes are recognised:
...
Norwegian cohorts use a mix of Norwegian-language labels and international codes:
...
| `creatinine` | `kreatiniini`, `p_kreatiniini` | (double i!) |
...
For EHR-derived data modelled on the OMOP Common Data Model, concept codes in
`LOINC_XXXX_X` format are recognised for all major analytes (hyphen in the
LOINC code is replaced by underscore):
