...
Most indices returned by `adipo_is()` are **originally described in the literature as insulin RESISTANCE markers** (higher values = worse metabolic health). To align with the package's sensitivity-centric philosophy, we have **algebraically inverted** these indices so that **more negative values indicate worse insulin resistance** (equivalently, lower insulin sensitivity).

### Inversion Strategy

For indices that measure insulin resistance (IR), we apply the transformation:

$$\text{Index}_{\text{inv}} = -\text{Index}_{\text{IR}}$$

This ensures a consistent interpretation: **more negative = worse adipose insulin sensitivity**.
...
- **Revised_QUICKI** and **McAuley_index** are retained in their original orientation because they are **already described as insulin SENSITIVITY indices** (higher values = better sensitivity). No inversion is applied.

### Practical Interpretation

In plots and models:
- More **negative** values of `*_inv` indices → **worse** adipose insulin sensitivity
- More **positive** values of `Revised_QUICKI` or `McAuley_index` → **better** insulin sensitivity

When combining multiple indices for clustering or PCA, use `normalize = "z"` to put all indices on a comparable scale before analysis.
