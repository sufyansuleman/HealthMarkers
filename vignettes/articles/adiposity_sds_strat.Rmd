...
## Scope
Sex-stratified SDS (z-scores) for anthropometric measures using separate male/female references, with NA policies and flexible variable mapping.
...
`adiposity_sds_strat()` returns a tibble of **only the `<var>_SDS` columns** — not the original data. Use `dplyr::bind_cols()` to rejoin.
...
Spot-check row 1 (male): BMI_SDS = (25.2 − 24.5) / 3.8 = `r round((25.2-24.5)/3.8, 4)`. Row 2 (female): BMI_SDS = (21.8 − 22.1) / 4.2 = `r round((21.8-22.1)/4.2, 4)`.
...
Use `prefix` to namespace output columns when combining multiple calls on the same data frame:
...
2. **Column map** — confirms which data column each mapped variable resolved to.
   Example: `adiposity_sds_strat(): column map: BMI -> 'BMI', waist -> 'waist', sex -> 'sex', ...`
3. **Results summary** — shows how many rows computed successfully (non-NA) per output column.
   Example: `adiposity_sds_strat(): results: BMI_SDS 28/30, waist_SDS 30/30, ...`

`verbose = TRUE` emits at the `"inform"` level; `options(healthmarkers.verbose = "inform")` must also be active:
...
- Forgetting `allow_partial = TRUE` when some variables are absent will error; enable it to compute what’s available.
